...
// Handle GET request to fetch new order items with cooking status 'pending'
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    // SQL query to fetch orders and order items where cooking_status is 'pending'
    $sql = "
        SELECT orders.*, order_items.* 
        FROM orders 
        INNER JOIN order_items ON orders.order_id = order_items.order_id 
        WHERE orders.cooking_status = 'pending'
    ";
